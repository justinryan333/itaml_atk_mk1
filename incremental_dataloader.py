...
class IncrementalDataset:
    ...
    def _setup_data(self, datasets, path, random_order=False, seed=1, increment=10, validation_split=0.):
        ...
        for dataset in datasets:
            if self.dataset_name == "cifar10poison":
                # Construct full paths
                train_file_path = os.path.join(path, "poison_datasets", "poisoned_train_set.pth")
                test_file_path = os.path.join(path, "poison_datasets", "poisoned_test_set.pth")

                # Print absolute paths for debugging
                print(f"Looking for train dataset at: {os.path.abspath(train_file_path)}")
                print(f"Looking for test dataset at: {os.path.abspath(test_file_path)}")

                # Check if files exist before loading
                if not os.path.exists(train_file_path):
                    raise FileNotFoundError(f"Train dataset not found: {os.path.abspath(train_file_path)}")
                if not os.path.exists(test_file_path):
                    raise FileNotFoundError(f"Test dataset not found: {os.path.abspath(test_file_path)}")

                # Load the poisoned datasets
                poisoned_train_data = torch.load(train_file_path, weights_only=False)
                print(f"Poisoned train data: {type(poisoned_train_data)}")
                poisoned_test_data = torch.load(test_file_path, weights_only=False)
                print(f"Poisoned test data: {type(poisoned_test_data)}")

                # Ensure that data and targets are in the correct type and format
                if isinstance(poisoned_train_data, torch.utils.data.ConcatDataset):
                    train_data_list = []
                    train_targets_list = []
                    for dataset in poisoned_train_data.datasets:
                        if isinstance(dataset.tensors[0], np.ndarray):
                            train_data_list.append(torch.tensor(dataset.tensors[0], dtype=torch.float32))
                        else:
                            train_data_list.append(dataset.tensors[0])
                        if isinstance(dataset.tensors[1], np.ndarray):
                            train_targets_list.append(torch.tensor(dataset.tensors[1], dtype=torch.long))
                        else:
                            train_targets_list.append(dataset.tensors[1])
                    train_data = torch.cat(train_data_list, dim=0)
                    train_targets = torch.cat(train_targets_list, dim=0)
                else:
                    if isinstance(poisoned_train_data, torch.utils.data.TensorDataset):
                        train_data = poisoned_train_data.tensors[0]
                        train_targets = poisoned_train_data.tensors[1]
                    else:
                        raise TypeError(f"Unsupported train dataset type: {type(poisoned_train_data)}")

                if isinstance(poisoned_test_data, torch.utils.data.ConcatDataset):
                    test_data_list = []
                    test_targets_list = []
                    for dataset in poisoned_test_data.datasets:
                        if isinstance(dataset.tensors[0], np.ndarray):
                            test_data_list.append(torch.tensor(dataset.tensors[0], dtype=torch.float32))
                        else:
                            test_data_list.append(dataset.tensors[0])
                        if isinstance(dataset.tensors[1], np.ndarray):
                            test_targets_list.append(torch.tensor(dataset.tensors[1], dtype=torch.long))
                        else:
                            test_targets_list.append(dataset.tensors[1])
                    test_data = torch.cat(test_data_list, dim=0)
                    test_targets = torch.cat(test_targets_list, dim=0)
                else:
                    if isinstance(poisoned_test_data, torch.utils.data.TensorDataset):
                        test_data = poisoned_test_data.tensors[0]
                        test_targets = poisoned_test_data.tensors[1]
                    else:
                        raise TypeError(f"Unsupported test dataset type: {type(poisoned_test_data)}")

                # Debugging: check types and indices
                print(f"Train data type: {type(train_data)}")
                print(f"Train targets type: {type(train_targets)}")
                print(f"Test data type: {type(test_data)}")
                print(f"Test targets type: {type(test_targets)}")

                # Create the custom datasets for training and testing
                self.train_dataset = torch.utils.data.TensorDataset(train_data, train_targets)
                self.test_dataset = torch.utils.data.TensorDataset(test_data, test_targets)

            else:
                # Handle other datasets (e.g., CIFAR10, CIFAR100, etc.)
                ...
                elif self.dataset_name in ["cub200", "cifar10", "cifar100", "mnist", "caltech101", "omniglot", "celeb"]:
...
class iCIFAR10POISON(DataHandler):
    base_dataset = None  # No direct torchvision dataset
